...
class API {
  ...
  async getDOIBaseInfo(DOI: string): Promise<ItemBaseInfo | undefined> {
    const head = {'x-api-key': 'UInvJexiUm39sANjHPX3GOG887qU5Qw5dwAiSSl1'}
    ...
    for (let route in routes) {
      let response = await this.requests.get(routes[route], 'json',head)
      if (response) {
        response.DOI = DOI
  ...
  async getDOIInfoBySemanticscholar(DOI: string): Promise<ItemInfo | undefined> {
    ...
    let response = await this.requests.get(api, 'json',head)
    if (response) {
      response.DOI = DOI
  ...
  async getDOIRelatedArray(DOI: string, limit: number = 20): Promise<ItemBaseInfo[] | undefined> {
    ...
    ,'x-api-key': 'UInvJexiUm39sANjHPX3GOG887qU5Qw5dwAiSSl1'})
